...
import { TopGroupItem } from "@/types/analytics.types"
...
export function TopGroups({ groups = [], loading = false }: TopGroupsProps) {
  return (
      ...
      <CardContent className="p-5 pt-0 space-y-4">
        {loading ? (
          Array.from({ length: 3 }).map((_, idx) => (
            <div key={idx} className="h-10 animate-pulse rounded-lg bg-muted/20" />
          ))
        ) : groups.length === 0 ? (
          <p className="text-xs text-muted-foreground italic py-6 text-center">
            No contact group metrics available.
          </p>
        ) : (
