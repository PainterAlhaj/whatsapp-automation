...
import { TopTemplateItem } from "@/types/analytics.types"
...
export function TopTemplates({ templates = [], loading = false }: TopTemplatesProps) {
  return (
      ...
      <CardContent className="p-5 pt-0 space-y-4">
        {loading ? (
          Array.from({ length: 3 }).map((_, idx) => (
            <div key={idx} className="h-12 animate-pulse rounded-lg bg-muted/20" />
          ))
        ) : templates.length === 0 ? (
          <p className="text-xs text-muted-foreground italic py-6 text-center">
            No template usage records available.
          </p>
        ) : (
