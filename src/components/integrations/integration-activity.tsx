...
import { mockIntegrationActivityData } from "@/lib/mock-data";
import { Cable, RefreshCw, Key, Link2Off, ShieldCheck } from "lucide-react";
...
export function IntegrationActivity({ integrationData }: IntegrationActivityProps) {
  const getActivityIcon = (action: string) => {
    ...
    } else if (act.includes("connect") || act.includes("authorize")) {
      return <Cable className="h-3.5 w-3.5 text-emerald-600 dark:text-emerald-400" />;
    } else if (act.includes("sync")) {
      return <RefreshCw className="h-3.5 w-3.5 text-blue-600 dark:text-blue-400" />;
    } else if (act.includes("regenerate") || act.includes("key")) {
      return <Key className="h-3.5 w-3.5 text-amber-600 dark:text-amber-400" />;
    } else {
      return <Link2Off className="h-3.5 w-3.5 text-red-600 dark:text-red-400" />;
    }
  };

  const activityList = React.useMemo(() => {
    const list = [...mockIntegrationActivityData];
    if (integrationData) {
    ...
    <Card className="border-border/80 shadow-xs text-left font-sans text-xs">
      <CardHeader className="p-5 pb-3">
        ...
        <CardDescription className="text-[11px]">
          Audit log of third-party platform authorizations and webhook synchronization sessions.
        </CardDescription>
      </CardHeader>
